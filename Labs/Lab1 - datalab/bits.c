...
int isGreater(int x, int y) {
  int equal, difference, xMSB, yMSB, sameSign, diffMSB;
  equal = !(x ^ y);
  difference = (x + ~y + 1);
  xMSB = (x >> 31) & 0x1; 
  yMSB = (y >> 31) & 0x1;
  sameSign = !(xMSB ^ yMSB); 
  diffMSB = (difference >> 31) & 0x1;
  return ((!diffMSB) & sameSign & (!equal)) | (yMSB & (!sameSign));
}
//7
/*
